    ...
    public class WallGrab : CustomComponentBase
    {
        ...
        public float WallSlide(float playerDir) //handles both wall sliding and wall jumping
        {
            ...
            if (wallDir != 0)
            {
                ...
                if (wallDir == 1)
                {
                    ...
                    else
                    {
                        ...
                        transform.position = new Vector3(transform.position.x + (wallBuffer * -1), transform.position.y, transform.position.z);
                ...
                if (wallDir == -1)
                {
                    ...
                    else
                    {
                        ...
                        //	PlayerMovement.moveVector = new Vector2 ((wallBuffer * -1f), PlayerMovement.verticleSpeed);	
                        transform.position = new Vector3(transform.position.x + (wallBuffer), transform.position.y, transform.position.z);
                        PlayerMovement.overrideInput = false;
                    }
                }
            }
            ...
            if (notWall != 0)
            {
                ...
                transform.position = new Vector3(transform.position.x + (notWall * -1f * wallslideBuffer), transform.position.y, transform.position.z);
            }
            else
            {
        ...
        float castRays()
        {
            ...
            { //if we are not on either wall then set walled to false to be used in other sections of controller
                notWall = 0;
                return 0f;
            }
